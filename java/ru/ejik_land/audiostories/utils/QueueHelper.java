...
package ru.ejik_land.audiostories.utils;
...
import static ru.ejik_land.audiostories.utils.MediaIDHelper.MEDIA_ID_MUSICS_BY_GENRE;
...
public class QueueHelper {
    ...
    public static final List<MediaSession.QueueItem> getPlayingQueue(String mediaId,
        ...
        if ((!category[0].equals(MEDIA_ID_MUSICS_BY_GENRE) && !category[0].equals(MEDIA_ID_MUSICS_BY_ALBUM)) || category.length != 2) {
            LogHelper.e(TAG, "Could not build a playing queue for this mediaId: ", mediaId, " Category[0]: ", category[0]);
            return null;
        }
        ...
        if(category[0].equals(MEDIA_ID_MUSICS_BY_GENRE)) {
            LogHelper.e(TAG, "Creating playing queue for musics of genre ", categoryValue);

            queue = convertToQueue(
                    musicProvider.getMusicsByGenre(categoryValue));

        } else if(category[0].equals(MEDIA_ID_MUSICS_BY_ALBUM)) {
            LogHelper.e(TAG, "Creating playing queue for musics of album ", categoryValue);

            queue = convertToQueue(
                    musicProvider.getMusicsByAlbum(categoryValue));
        }
